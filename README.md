...
| 4 | If you run the script more often than once in 120 minutes the output of **Current Configs in Archive** (./Modules/Archive/Checker.sh) will not be correct | Due to a setting in the modul. Change ```-mmin -120``` to a lower value e.g ```-mmin -30```. This can cause problems getting all configs if they are olded than the defined value (Will show say less **Current Configs in Archive** than realy backuped.)| More likely yes
| 5 | If the backup process takes more than 120 minutes the output of ./Modules/Archive/ArchiveStats.sh and ./Modules/Archive/Checker.sh will not be correct | Due to settings in the modules. Change ```-mmin -120``` to a higher value e.g ```-mmin -360``` to fix in ./Modules/Archive/ArchiveStats.sh **Total lines operating firewalls** and **Average lines in conifig file** and in ./Modules/Archive/Checker.sh will **Current Configs in Archive** get fixed and will be displayed correctly | No (Can be fixed manually) |
